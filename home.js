...
fetch("https://dummyjson.com/products")
    ...
    .then((data) => {
        ...
        for (var i = 0; i < final.length; i++) {
            lis.push(
                final[i].brand.toLowerCase() + final[i].title.toLowerCase()
            );
        }
        let x = "";
        final.map((ele) => {
...
function searchh() {
    ...
    for (var i = 0; i < m.length; i++) {
        for (var j = 0; j < glob.length; j++) {
            if (
                m[i] ===
                glob[j].brand.toLowerCase() + glob[j].title.toLowerCase()
            ) {
                ans.push(glob[j]);
            }
        }
...
function addClicked(val) {
    ...
    if (flag === 0) {
        cart.push({
            id: val,
            title: glob[val].title,
            brand: glob[val].brand,
            count: parseInt(check) + 1,
        });
    }
    printCart();
}
